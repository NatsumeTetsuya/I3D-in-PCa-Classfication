...
def parse_args():
    ...
    parser.add_argument('config', type=str, default='./config.py', help='test config file path')
    parser.add_argument('checkpoint', type=str, default="/home/chenpeizhe/project/guokeda/dongyang/classfication/MMaction/results/epoch_5.pth", help='checkpoint file/url')
    parser.add_argument('video', type=str, default="/home/chenpeizhe/Dataset/dongyang_plus/clips_preprocess256/00022/"
...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        results = gradcam(data, use_labels=False, alpha=0.5)
        ...
        if args.out_folder is not None:
            ...
            frames = frames_batches.reshape(-1, *frames_batches.shape[-3:])#-3

            for j in range(frames.shape[0]):

                cv2.imwrite(os.path.join(args.out_folder,'{}.png'.format(str(j+i*32+1).zfill(5))),frames[j])
